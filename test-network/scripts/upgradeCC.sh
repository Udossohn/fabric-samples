...
upgradeChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ] ; then
    set -x
     peer chaincode upgrade -o localhost:7050 --channelID $CHANNEL_NAME --name fabcar --version ${VERSION} -c '{"function":"initLedger","Args":[]}'
     
    set +x
  else
    set -x
    peer chaincode upgrade -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name fabcar --version ${VERSION} -c '{"function":"initLedger","Args":[]}'
    set +x
  fi
}
